...
## Why Exact LOO (and not an approximation)?

Influence functions and Cook's-distance-style diagnostics estimate the effect of removing a point using gradients at the fitted solution, without retraining. That's much faster, but it's an approximation. Because the sample size here (1,000) and model (logistic regression on 384-dim embeddings) make full retraining cheap - 800 refits in about 14 seconds - this project uses exact LOO instead, so every influence score reflects an actual retrained model rather than a first-order estimate.
